...
prdg_vertex_p scc::new_prdg_vertex (unsigned int p, int max)
{
  prdg_vertex_p v;
  ...
  PRDGV_PRED (v) = NULL;
  PRDGV_SCC (v) = 0;
  PRDGV_PV (v) = new (std::vector<rdg_vertex_p>);
//  PRDGV_PV (v) = XNEW(std::vector<rdg_vertex_p>);
//  PRDGV_PV (v) = XCNEWVEC(std::vector<rdg_vertex_p>, max);
  return v;
}
...
prdg_p scc::new_prdg (rdg_p rdg)
{
  ...
  PRDG_RDG (rdgp) = rdg;
//  PRDG_V (rdgp) = XCNEWVEC(std::vector<prdg_vertex_p>, rdg->nb_vertices);
//  PRDG_E (rdgp) = XCNEWVEC(std::vector<prdg_edge_p>, rdg->nb_edges);
  PRDG_V (rdgp) = new std::vector<prdg_vertex_p>;
  PRDG_E (rdgp) = new std::vector<prdg_edge_p>;
  ...
  for (i = 0; i < nb_sccs; i++)
    {
     ...
     unsigned int nbv = RDG_NBV (PRDG_RDG (sccg));
      prdg_vertex_p v = new_prdg_vertex (current_scc, g->rdg->nb_vertices);
...
//  vertices = XCNEWVEC (prdg_vertex_p, max_f+1);
  vertices = new prdg_vertex_p[max_f+1];
...
//  errs() << "creating vertices...\n";
  ...
  errs() << i << " ...\n";
      ...
      prdg_vertex_p v = new_prdg_vertex (current_partition, rdg->nb_vertices);
      
  errs() << "123: " << rdg->nb_vertices << " ...\n";
...
//  RDG_V (rdg) = XCNEWVEC (struct rdg_vertex, RDG_NBV (rdg));
  RDG_V (rdg) = new rdg_vertex;
  
  vertex_index = 0;
  std::vector<BasicBlock*> bb = loop_nest->getBlocks();
  BasicBlock* bb_p;
  BasicBlock* Header = loop_nest->getHeader();

  errs() << "num of blocks: " << loop_nest->getNumBlocks() << "\n"; 
...
/*
              ...
              RDGV_PARTITIONS (v) = XCNEWVEC(std::vector<int>, (RDG_NBV (rdg)));
              RDGV_SCCS (v) = XCNEWVEC(std::vector<int>, (RDG_NBV (rdg)));
*/
...
//  RDG_E (rdg) = XCNEWVEC (struct rdg_edge, RDG_NBE (rdg));
  RDG_E (rdg) = new rdg_edge;

  /* Create data edges.  */
  edge_index = 0;
 
  errs() << "dep_r->size()" << dep_r->size() << "\n";
...
void
scc::dump_rdg (FILE *outf, rdg_p rdg)
{
  unsigned int i;
  rdg_vertex_p vertex;
  ...
  for (i = 0; i < RDG_NBV (rdg); i++)
    { 
      ...
      fprintf (outf, "S%d : ", RDGV_N (v));
// TODO: how to output an instruction??
...
//      print_generic_expr (outf, RDGV_INSTRS (vertex), 0);
      fprintf (outf, "<instruction%s>", RDGV_INSTRS (vertex));
...
void scc::out_scc(std::vector<prdg_vertex_p> scc, Loop *loop_nest)
{
